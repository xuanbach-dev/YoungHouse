...
Write-Host "Optimizing Young House 6-30 images..." -ForegroundColor Green
...
# Set directories
$SourceDir = "public\rooms\branch-6\Type30"
$BackupDir = "public\rooms-backup\branch-6\Type30"
...
# Image files to process (include the typo filename exactly as present)
$ImageFiles = @(
    "brranch6-30-1.jpg",
    "branch6-30-2.jpg",
    "branch6-30-3.jpg"
...
foreach ($ImageFile in $ImageFiles) {
    ...
    if (Test-Path $SourcePath) {
        ...
        # Create optimized JPG (ensure .jpg, strip metadata, progressive)
        ...
        if ($Verbose) { Write-Host "  Creating optimized JPG: $BaseName.jpg" -ForegroundColor Gray }
        & magick @MagickArgs
        ...
        if ($Verbose) { Write-Host "  Creating thumbnail JPG: $BaseName.thumb.jpg" -ForegroundColor Gray }
        & magick @ThumbArgs
        ...
        if ($CreateWebP) {
            ...
            if ($Verbose) { Write-Host "  Creating WebP: $BaseName.webp" -ForegroundColor Gray }
            & magick @WebPArgs
            ...
            if ($Verbose) { Write-Host "  Creating WebP thumbnail: $BaseName.thumb.webp" -ForegroundColor Gray }
        ...
        if ($CreateAVIF) {
            try {
                ...
                if ($Verbose) { Write-Host "  Creating AVIF: $BaseName.avif" -ForegroundColor Gray }
                & magick @AVIFArgs
            } catch {
        ...
        Write-Host ("  Optimized {0} ({1}% size reduction)" -f $ImageFile, $SizeReduction) -ForegroundColor Green
...
if ($TotalSizeBefore -gt 0) {
    ...
    Write-Host ("Total size reduction: {0}% ({1}MB -> {2}MB)" -f $TotalReduction, $SizeBeforeMB, $SizeAfterMB) -ForegroundColor Cyan
}
...
Write-Host "  - Optimized JPG files (quality $Quality%)" -ForegroundColor Gray
Write-Host "  - Thumbnail JPG files ($ThumbnailSize, quality 80%)" -ForegroundColor Gray
if ($CreateWebP) {
    Write-Host "  - WebP files (quality $Quality%)" -ForegroundColor Gray
    Write-Host "  - WebP thumbnail files ($ThumbnailSize, quality 80%)" -ForegroundColor Gray
}
if ($CreateAVIF) {
    Write-Host "  - AVIF files (if supported)" -ForegroundColor Gray
}
